...
import adminCaptchaService from '../services/adminCaptchaService.js'
import { strongPasswordSchema } from '../utils/validator.js'

const router = Router()

const adminCaptchaLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 40,
...
// 所有接口都需要管理员权限
// 管理员登录图形验证码（无需认证）
router.get('/auth/captcha', adminCaptchaLimiter, async (req, res, next) => {
  try {
    const data = await adminCaptchaService.createCaptcha()
    success(res, data)
  } catch (err) {
    logger.error('[admin] 生成登录验证码失败:', err)
...
router.post('/auth/login', adminLoginLimiter, async (req, res, next) => {
  try {
    ...
    let captchaCheck
    try {
      captchaCheck = await adminCaptchaService.verifyCaptcha(captchaId, captchaCode)
    } catch (captchaErr) {
      logger.error('[admin] 校验登录验证码异常:', captchaErr)
      return res.status(503).json({ code: 503, message: '验证码服务异常，请刷新验证码后重试', data: null })
    }
    if (!captchaCheck.ok) {
    ...
    try {
      ...
      try {
        const loginLogService = (await import('../services/loginLogService.js')).default
        ...
        logger.warn('管理员登录日志写入失败:', e.message)
      }
      ...
      try {
        ...
        await loginLogService.log({
          ...
          loginStatus: 'failed',
          failureReason: loginErr.message || '登录失败',
          req
        })
      } catch (e) {
        logger.warn('管理员失败登录日志写入失败:', e.message)
      }
      throw loginErr
    }
  } catch (err) {
...
router.get('/stats/trend', async (req, res, next) => {
  try {
    ...
    const trendDays = parseInt(days, 10) || 7
    const trendData = await statisticsService.getTrendData(trendDays)
...
router.get('/stats/today-realtime-trend', async (req, res, next) => {
  try {
    const { interval = 10 } = req.query
    const intervalMinutes = parseInt(interval, 10) || 10
    const trendData = await statisticsService.getTodayRealtimeTrend(intervalMinutes)
    res.set('Cache-Control', 'no-store')
    success(res, trendData)
  } catch (err) {
    next(err)
...
router.put('/users/:id/password', async (req, res, next) => {
  try {
    ...
    const parsedPwd = strongPasswordSchema.safeParse(password)
    if (!parsedPwd.success) {
      const msg = parsedPwd.error.errors.map((e) => e.message).join('; ')
      return res.status(400).json({ code: 400, message: msg, data: null })
    }
...
router.get('/task-claim-config', async (req, res, next) => {
  try {
    ...
    // 获取曝光配置（统一使用运行时结构，避免和主配置页出现两套口径）
    ...
    const result = {
      ...
      system: {
        task_timeout_minutes: parseInt(systemConfig.defaultTimeLimitMinutes || systemConfig.task_timeout_minutes || '10'),
        max_time_limit_minutes: parseInt(systemConfig.task_default_time_limit || systemConfig.max_time_limit_minutes || '15'),
        max_concurrent_per_user: parseInt(systemConfig.maxActiveTasksPerUser || systemConfig.max_concurrent_per_user || '5'),
        city_limit_per_task: parseInt(systemConfig.task_default_city_limit || systemConfig.city_limit_per_task || '1'),
        province_limit_per_task: parseInt(systemConfig.task_default_province_limit || systemConfig.province_limit_per_task || '4'),
        // 兼容旧聚合接口调用方，后续可逐步移除
        defaultTimeLimitMinutes: parseInt(systemConfig.defaultTimeLimitMinutes || systemConfig.task_timeout_minutes || '10'),
        maxConcurrentPerUser: parseInt(systemConfig.maxActiveTasksPerUser || systemConfig.max_concurrent_per_user || '5'),
        cityLimitPerTask: parseInt(systemConfig.task_default_city_limit || systemConfig.city_limit_per_task || '1'),
        provinceLimitPerTask: parseInt(systemConfig.task_default_province_limit || systemConfig.province_limit_per_task || '4'),
...
router.put('/task-claim-config/system', async (req, res, next) => {
  try {
    ...
    // 更新配置：统一走 adminService.updateSystemConfig，保持和主页面同一套 key 映射规则
    const updates = []
    const configMap = {
      task_timeout_minutes: data.task_timeout_minutes ?? data.defaultTimeLimitMinutes,
      max_time_limit_minutes: data.max_time_limit_minutes,
      max_concurrent_per_user: data.max_concurrent_per_user ?? data.maxConcurrentPerUser,
      city_limit_per_task: data.city_limit_per_task ?? data.cityLimitPerTask,
      province_limit_per_task: data.province_limit_per_task ?? data.provinceLimitPerTask,
      pointsToYuan: data.pointsToYuan,
      minWithdrawAmount: data.minWithdrawAmount
    }
    
    for (const [key, rawValue] of Object.entries(configMap)) {
...
router.put('/task-claim-config/exposure', async (req, res, next) => {
  try {
    ...
    const oldConfig = await exposureService.getConfig()
    const config = await exposureService.updateConfig(data)
...
function safeParseTaskConfigJson(value, fallback) {
...
router.get('/task-default-config', async (req, res, next) => {
  try {
    ...
    success(res, {
      ...
      requirements: safeParseTaskConfigJson(configMap.task_default_requirements, []),
...
router.post('/task-default-config', async (req, res, next) => {
  try {
    ...
    const qualificationStandards = Array.isArray(tutorialConfig.qualificationStandards)
      ? tutorialConfig.qualificationStandards
      : DEFAULT_TASK_TUTORIAL_CONFIG.qualificationStandards
    const positiveCommentExamples = Array.isArray(tutorialConfig.positiveCommentExamples)
      ? tutorialConfig.positiveCommentExamples
      : DEFAULT_TASK_TUTORIAL_CONFIG.positiveCommentExamples
    const commentLibrary = Array.isArray(tutorialConfig.commentLibrary)
      ? tutorialConfig.commentLibrary
      : DEFAULT_TASK_TUTORIAL_CONFIG.commentLibrary
    const copyTip = typeof tutorialConfig.copyTip === 'string' && tutorialConfig.copyTip.trim()
      ? tutorialConfig.copyTip.trim()
      : DEFAULT_TASK_TUTORIAL_CONFIG.copyTip
    ...
    const updates = [
      ...
      { key: 'task_tutorial_qualification_standards', value: JSON.stringify(qualificationStandards) },
      { key: 'task_tutorial_positive_comment_examples', value: JSON.stringify(positiveCommentExamples) },
      { key: 'task_tutorial_comment_library', value: JSON.stringify(commentLibrary) },
      { key: 'task_tutorial_copy_tip', value: copyTip }
    ]
...
router.get('/withdrawals/stats', async (req, res, next) => {
  try {
    const results = await Promise.all([
      ...
      supabase.from('withdrawals').select('*', { count: 'exact', head: true }).eq('status', 'paid'),
    ])
    success(res, {
    ...
    success(res, { total: 0, pending: 0, approved: 0, paid: 0 })
  }
})
...
// ============ 用户注册码（与推广邀请码分离）============
function generateRegistrationBatchCode() {
...
function buildRegistrationPolicyFromRows(rows) {
  const map = {}
  for (const r of rows || []) {
    map[r.key] = r.value
  }
  const registrationCodeRequired = map.register_code_required === 'true'
  const inviteRequired = map.c_promotion_invite_required === 'true'
  return {
...
router.get('/registration-codes/policy', async (req, res, next) => {
  try {
    const rows = await prisma.$queryRaw`
      SELECT key, value FROM system_configs
      WHERE key IN ('register_code_required', 'c_promotion_invite_required')
    `
    success(res, buildRegistrationPolicyFromRows(rows))
  } catch (err) {
    next(err)
  }
})

router.put('/registration-codes/policy', async (req, res, next) => {
  try {
    const { required, inviteRequired } = req.body || {}

    if (required !== undefined) {
      const val = required === true || required === 'true' ? 'true' : 'false'
      await prisma.$executeRaw`
        INSERT INTO system_configs (key, value, description, created_at, updated_at)
        VALUES ('register_code_required', ${val}, '开启后用户必须填写有效注册码才能注册', NOW(), NOW())
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()
      `
      await logOperation(req, 'update', 'config', 'register_code_required', '', '', val, '注册码必填策略')
    }

    if (inviteRequired !== undefined) {
      const val = inviteRequired === true || inviteRequired === 'true' ? 'true' : 'false'
      await prisma.$executeRaw`
        INSERT INTO system_configs (key, value, description, created_at, updated_at)
        VALUES ('c_promotion_invite_required', ${val}, '开启后用户注册必须填写推广邀请码', NOW(), NOW())
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()
      `
      await logOperation(req, 'update', 'config', 'c_promotion_invite_required', '', '', val, '推广邀请码必填策略')
    }
...
router.get('/registration-codes', async (req, res, next) => {
  try {
    const page = Math.max(1, parseInt(req.query.page, 10) || 1)
    const size = Math.min(100, Math.max(1, parseInt(req.query.size, 10) || 20))
    const offset = (page - 1) * size
    const rows = await prisma.$queryRaw`
      SELECT id, code, note, max_uses, used_count, expires_at, disabled, created_by, created_at
      FROM registration_codes
      ORDER BY created_at DESC
      LIMIT ${size} OFFSET ${offset}
    `
    const countRow = await prisma.$queryRaw`SELECT COUNT(*)::int AS c FROM registration_codes`
...
router.post('/registration-codes', async (req, res, next) => {
  try {
    const count = Math.min(50, Math.max(1, parseInt(req.body?.count, 10) || 1))
    const maxUses = Math.max(1, parseInt(req.body?.maxUses, 10) || 1)
    ...
    for (let i = 0; i < count; i++) {
      let inserted = false
      for (let tries = 0; tries < 15 && !inserted; tries++) {
        const code = generateRegistrationBatchCode()
        try {
          if (expiresAt) {
            await prisma.$executeRaw`
              INSERT INTO registration_codes (code, note, max_uses, used_count, disabled, created_by, expires_at, created_at)
              VALUES (${code}, ${note}, ${maxUses}, 0, false, ${adminName}, ${new Date(expiresAt)}, NOW())
            `
          } else {
            await prisma.$executeRaw`
              INSERT INTO registration_codes (code, note, max_uses, used_count, disabled, created_by, created_at)
              VALUES (${code}, ${note}, ${maxUses}, 0, false, ${adminName}, NOW())
            `
          }
          created.push({ code })
          inserted = true
    ...
    await logOperation(req, 'create', 'registration_code', '', '', '', String(created.length), `批量生成注册码 ${created.length} 个`)
    success(res, { created, count: created.length })
  } catch (err) {
    next(err)
  }
})

router.patch('/registration-codes/:id', async (req, res, next) => {
  try {
    const id = BigInt(req.params.id)
    const { disabled } = req.body || {}
    if (disabled !== true && disabled !== 'true') {
      return res.status(400).json({ code: 400, message: '仅支持 disabled: true 作废注册码', data: null })
    }
    await prisma.$executeRaw`
      UPDATE registration_codes SET disabled = true WHERE id = ${id}
    `
    await logOperation(req, 'disable', 'registration_code', String(id), '', '', 'true', '作废注册码')
    success(res, { ok: true })
  } catch (err) {
    next(err)
  }
