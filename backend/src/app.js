...
const helmetMod = await import('helmet').catch(() => null)
const helmetMiddleware = helmetMod?.default
  ? helmetMod.default({
...
// API 使用 Authorization Bearer，不依赖 Cookie 会话；配合严格 CORS 可显著降低 CSRF 面（浏览器跨站无法读自定义头）。

// ============ 中间件配置 ============

if (!helmetMod?.default) {
  console.warn('[app] 未加载 helmet（请在后端目录执行 npm install），部分安全响应头未启用')
}
...
const apiMax = Math.max(50, parseInt(process.env.API_RATE_LIMIT_MAX || '', 10) || (process.env.NODE_ENV === 'production' ? 400 : 1000))
const authMax = Math.max(10, parseInt(process.env.AUTH_RATE_LIMIT_MAX || '', 10) || (process.env.NODE_ENV === 'production' ? 60 : 100))
...
function isHealthDetailAuthorized(req) {
  const token = process.env.HEALTH_DETAIL_TOKEN
  if (!token) return false
  const q = req.query?.token
  const h = req.headers['x-health-token']
  return q === token || h === token
}

function publicHealthPayload(snapshot, errMessage) {
  if (errMessage) {
    return {
      status: 'error',
...
// 健康检查（对外默认仅返回 status/timestamp；带 HEALTH_DETAIL_TOKEN 可查看详情：?token= 或 X-Health-Token）
app.get('/health', async (req, res) => {
  try {
    const snapshot = await buildHealthSnapshot()
    const detail = isHealthDetailAuthorized(req)
    const body = detail ? snapshot : publicHealthPayload(snapshot)
    res.status(snapshot.status === 'ok' ? 200 : 503).json(body)
  } catch (e) {
    ...
    res.status(503).json(
      detail
        ...
        : publicHealthPayload(null, e.message)
    )
  }
})
...
// API路由
// 用户端：验证码走 apiLimiter；登录在路由内单独 userLoginLimiter 防爆破
...
// 与 Nginx 将 /admin/api 直转后端时的路径对齐（免登录验证码、登录等）
app.use('/admin/api', apiLimiter, adminNewRoutes)
