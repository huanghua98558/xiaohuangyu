...
export class HomepageOcrWorker {
  constructor() {
    this.running = false;
    this.consumer = createImageReviewOcrConsumer({
      ocrProfile: 'homepage',
      workerName: 'homepageOcrWorker',
    });
  }

  async start() {
    console.log('[HomepageOcrWorker] 骨架模式启动');
    console.log('[HomepageOcrWorker] 当前支持常驻轮询 pending homepage OCR job，并在处理后刷新 merge 状态');
    this.running = true;
    this.poll();
  }
  ...
  async poll() {
    while (this.running) {
      try {
        ...
        console.log('[HomepageOcrWorker] OCR job 已处理:', JSON.stringify({
          dispatchKey: result.job?.dispatch_key,
          status: result.status,
          mergeReady: result.mergeState?.mergePayload?.readiness?.ready || false,
        }));
      } catch (error) {
        console.error(`[HomepageOcrWorker] 轮询处理失败: ${error.message}`);
...
if (shouldAutoStartWorker()) {
  const worker = new HomepageOcrWorker();
