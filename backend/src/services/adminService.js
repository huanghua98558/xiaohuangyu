...
import { clearSystemConfigCache, setConfigValue } from './systemConfigService.js'
...
const SYSTEM_CONFIG_ALIAS_MAP = {
  task_timeout_minutes: {
    targetKey: 'defaultTimeLimitMinutes',
    description: '领取默认超时（分钟）'
  },
  max_time_limit_minutes: {
    targetKey: 'task_default_time_limit',
    description: '发布默认时限（分钟）'
  },
  max_concurrent_per_user: {
    targetKey: 'maxActiveTasksPerUser',
    description: '全局默认并发任务数（等级未配置时兜底）'
  },
  city_limit_per_task: {
    targetKey: 'task_default_city_limit',
    description: '发布默认每城市限制人数'
  },
  province_limit_per_task: {
    targetKey: 'task_default_province_limit',
    description: '发布默认每省份限制人数'
  }
}
...
class AdminService {
  ...
  async adminLogin(username, password) {
    ...
    if (users && users.length > 1) {
      logger.warn(`管理员登录标识命中多条记录，已取 id 最大一条: key=${key}`)
    }
  ...
  async getStats() {
    ...
    const [
      ...
      r_blockedAll, r_blockedToday, r_suspected, r_confirmed,
    ] = await Promise.all([
      prisma.$queryRaw`
        SELECT COUNT(*)::int AS c FROM claims c
      ...
      supabase.from('blocked_accounts').select('*', { count: 'exact', head: true }).in('status', ['suspected', 'confirmed']).gte('created_at', today.toISOString()).lt('created_at', tomorrow.toISOString()),
    ...
    const totalCompletedClaims = Number(r_dualTotal[0]?.c || 0)
    ...
    const todayCompletedClaims = Number(r_dualToday[0]?.c || 0)
    const yesterdayCompletedClaims = Number(r_dualYesterday[0]?.c || 0)
    const dayBeforeCompletedClaims = Number(r_dualDayBefore[0]?.c || 0)
    ...
    // 「今日完成次数」与「今日完成任务」均按双审通过 + 完成时刻统计（与任务表 status=completed 脱钩）
    const todayCompletedTasksCount = todayCompletedClaims
    const yesterdayCompletedTasksCount = yesterdayCompletedClaims
    const dayBeforeCompletedTasksCount = dayBeforeCompletedClaims

    // 总积分发放应为所有正向积分流水总和，而不是仅任务完成积分
    const totalPointsIssued = Number(r_totalPointsIssued[0]?.total || 0)
    ...
    const result = {
      ...
      todayBlockedAccounts: r_blockedToday.count || 0,
  ...
  async getTrendData(days = 7) {
    ...
    const result = await statisticsService.getTrendData(days)
  ...
  async getSystemConfigs() {
    ...
    const aliasKeys = new Set(Object.keys(SYSTEM_CONFIG_ALIAS_MAP))

    const normalizedConfigs = (configs || []).filter((item) => !aliasKeys.has(item.key))

    for (const [aliasKey, meta] of Object.entries(SYSTEM_CONFIG_ALIAS_MAP)) {
      const targetConfig = configMap.get(meta.targetKey)
      const legacyConfig = configMap.get(aliasKey)
      normalizedConfigs.push({
        ...(targetConfig || legacyConfig || {}),
        key: aliasKey,
        value: targetConfig?.value ?? legacyConfig?.value ?? null,
  ...
  async updateSystemConfig(key, value) {
    const meta = SYSTEM_CONFIG_ALIAS_MAP[key]
    const targetKey = meta?.targetKey || key
    const description = meta?.description || null

    await setConfigValue(targetKey, value, { description })
    clearSystemConfigCache(targetKey)
    if (meta?.targetKey) {
      clearSystemConfigCache(key)
    }
    await cache.del('sys:config')

    logger.info(`管理员更新系统配置 ${key} -> ${targetKey} = ${value}`)
    return {
      key,
      value: String(value),
      description
  ...
  async getLogOverviewStats() {
    ...
    const reviewTotal = await prisma.task_review_logs.count()
    const anomalyTotal = await prisma.login_logs.count({
    ...
    return {
      ...
      operationTotal: totalOps,
      loginTotal: totalLogins,
      todayLogin: todayLogins,
      reviewTotal,
      anomalyTotal
    }
  }
