...
class PointsSettlementService {
  ...
  async getClaimContext(claimId) {
    return db.queryOne(
      ...
      SELECT 
        ...
        c.final_points,
        c.bonus_points,
        c.publish_time_snapshot,
        c.config_snapshot,
        c.settlement_snapshot,
  ...
  resolveBasePoints(ctx, explicitBasePoints) {
    if (explicitBasePoints !== undefined && explicitBasePoints !== null) {
      return Math.max(0, roundPoints(explicitBasePoints))
    }
    ...
    return Math.max(0, roundPoints(candidate || 0))
  }
  ...
  buildFallbackSnapshot(ctx, {
    ...
    const claimTime = ctx.claimed_at || new Date().toISOString()
    const publishAt = publishTime || ctx.task_created_at || new Date().toISOString()
    const breakdown = calculatePointBreakdown(basePoints, coefficientResult?.coefficient || 1)
    ...
    return {
      ...
      taskPublishedAt: publishAt,
      claimTime,
      lockTime: claimTime,
      publishTime: claimTime,
      previewType: 'locked',
  ...
  async awardClaimPoints({
    ...
    const lockedSnapshot = parseJsonObject(ctx.settlement_snapshot, {}) || {}
    const resolvedBasePoints = this.resolveBasePoints(
      ctx,
      lockedSnapshot.basePoints ?? basePoints
    )
    ...
    const hasLockedSnapshot = Number(lockedSnapshot.coefficient || 0) > 0
    const coefficientResult = hasLockedSnapshot
      ? null
      : await nightPointService.calculateCoefficientByPublishTime({
          publishTime: ctx.claimed_at || publishTime || ctx.task_created_at,
          onlineUsers,
          acceptedCount,
          needCount
        })

    const fallbackSnapshot = this.buildFallbackSnapshot(ctx, {
      claimId,
      taskId: finalTaskId,
      userId: finalUserId,
    ...
    const snapshot = {
      ...
      metadata,
      calculatedAt: new Date().toISOString(),
      onlineUsers: lockedSnapshot.onlineUsers ?? fallbackSnapshot.onlineUsers,
      onlineUsersSource: lockedSnapshot.onlineUsersSource ?? fallbackSnapshot.onlineUsersSource,
    }
    const display = buildSettlementDisplay(snapshot, ctx)
    const coefficient = display.coefficient
    const finalPoints = display.finalPoints
    const bonusPoints = display.bonusPoints
    const isNight = display.isNight
    const publishTimeForNotice =
      snapshot.taskPublishedAt || ctx.publish_time_snapshot || publishTime || ctx.task_created_at || null
    ...
    await db.transaction(async (client) => {
      ...
      if (claimColumns.has('publish_time_snapshot')) {
        ...
        values.push(snapshot.taskPublishedAt || publishTime || ctx.task_created_at || null)
      }
      if (claimColumns.has('config_snapshot')) {
        updates.push(`config_snapshot = $${values.length + 1}`)
        values.push(JSON.stringify(snapshot.config || null))
      }
    ...
    await publishPointsAwarded(finalUserId, finalPoints, {
      ...
      finalPoints,
      isNight,
      publishTime: publishTimeForNotice,
      onlineUsers
    })
