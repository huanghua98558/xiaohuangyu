...
export async function enqueueLinkVerificationCompat(params) {
  ...
  const normalizedSubmissionVersion = normalizeSubmissionVersion(submissionVersion, now)
...
export async function enqueueReview(claimId, options = {}) {
  try {
    ...
    let submissionVersion = options.submissionVersion
    
    if (!userId || !taskId) {
      // 直接走 CockroachDB，避免适配层类型偏差
      const claim = await db.queryOne(
        'SELECT id, user_id, task_id, screenshots, submitted_at FROM claims WHERE id = $1',
        [normalizedClaimId]
      )
      ...
      submissionVersion = submissionVersion || claim.submitted_at
    }
    ...
    const normalizedSubmissionVersion = normalizeSubmissionVersion(
      submissionVersion || options.submittedAt,
      nowIso
    )
    ...
    if (existingQueue) {
      ...
      queueItem = await db.queryOne(
        ...
        SET status = 'pending',
            screenshots = $3,
            submission_version = $4::timestamptz,
            ai_result = NULL,
            ai_confidence = NULL,
            ai_reason = NULL,
            processed_at = NULL,
        ...
        RETURNING *
        `,
        [normalizedClaimId, nowIso, screenshotsStr, normalizedSubmissionVersion]
      )
    } else {
      // 创建新记录
      ...
      queueItem = await db.queryOne(
        ...
        INSERT INTO ai_review_queue
          (claim_id, user_id, task_id, screenshots, submission_version, status, priority, retry_count, updated_at, created_at)
        VALUES
          ($1, $2, $3, $4, $5::timestamptz, 'pending', $6, 0, $7, $7)
        RETURNING *
        `,
        [normalizedClaimId, userIdInt, taskIdInt, screenshotsStr, normalizedSubmissionVersion, priority, nowIso]
      )
    }
    ...
    logger.info(`审核任务入队: claimId=${normalizedClaimId}, queueId=${queueItem.id}, submissionVersion=${normalizedSubmissionVersion}`)
