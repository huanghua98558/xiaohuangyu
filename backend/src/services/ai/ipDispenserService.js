...
const VALID_MODES = new Set(['direct', 'proxy', 'auto'])
...
class IPDispenserService {
  ...
  async init() {
    ...
    try {
      ...
      // 初始化统计（保留已有运行数据，避免服务重启后全部清零）
      const stats = await client.hGetAll(KEY.STATS)
      await client.hSet(KEY.STATS, {
        totalIPs: stats.totalIPs || '0',
        usedIPs: stats.usedIPs || '0',
        directRequests: stats.directRequests || '0',
        proxyRequests: stats.proxyRequests || '0',
        captchaHits: stats.captchaHits || '0',
        modeSwitches: stats.modeSwitches || '0'
      })
  ...
  async getConfiguredMode() {
    ...
    return VALID_MODES.has(CONFIG.DEFAULT_MODE) ? CONFIG.DEFAULT_MODE : 'direct'
  }
  ...
  async getCooldownRemaining() {
    ...
    return cooldownEnd ? Math.max(0, parseInt(cooldownEnd, 10) - Date.now()) : 0
  }

  async activateAutoProxy(reason, cooldownMinutes = CONFIG.DIRECT_COOLDOWN) {
    const client = await getRedisClient()
    if (!client) return false

    const configuredMode = await this.getConfiguredMode()
    ...
    logger.warn(`[IP发放] AUTO 模式切换到代理执行: ${reason}，冷却 ${cooldownMinutes} 分钟`)
    return true
  }

  async getCurrentMode(queueSize = 0) {
    const client = await getRedisClient()
    if (!client) return 'direct'
    ...
    if (queueSize >= CONFIG.QUEUE_THRESHOLD) {
      await this.activateAutoProxy(`队列积压 ${queueSize} >= ${CONFIG.QUEUE_THRESHOLD}`, CONFIG.DIRECT_COOLDOWN)
      return 'proxy'
    }

    const failCount = parseInt(await client.get(KEY.DIRECT_FAIL) || '0', 10)
    if (failCount >= CONFIG.DIRECT_FAIL_LIMIT) {
      await this.activateAutoProxy(`连续失败 ${failCount} >= ${CONFIG.DIRECT_FAIL_LIMIT}`, CONFIG.DIRECT_COOLDOWN)
      return 'proxy'
    }
  ...
  async shouldSwitchToProxy(queueSize) {
    ...
    return (await this.getCurrentMode(queueSize)) === 'proxy'
  }
  ...
  async reportDirectFailure() {
    ...
    if (failCount >= CONFIG.DIRECT_FAIL_LIMIT) {
      await this.activateAutoProxy(`连续失败 ${failCount} 次`, CONFIG.DIRECT_COOLDOWN)
    }
  }
  ...
  async reportCaptcha() {
    ...
    await this.activateAutoProxy('检测到验证码', CONFIG.CAPTCHA_COOLDOWN)
    
    logger.warn(`[IP发放] 遇到验证码，AUTO 模式进入代理执行，冷却 ${CONFIG.CAPTCHA_COOLDOWN} 分钟`)
  }
  ...
  async getIP(queueSize = 0) {
    await this.init()
    const client = await getRedisClient()
    const mode = await this.getCurrentMode(queueSize)
  ...
  async getPoolStatus() {
    ...
    const configuredMode = await this.getConfiguredMode()
    const effectiveMode = await this.getCurrentMode()
    const poolSize = await client.lLen(KEY.POOL)
    const failCount = parseInt(await client.get(KEY.DIRECT_FAIL) || '0')
    ...
    return {
      ...
      cooldownRemaining: await this.getCooldownRemaining(),
      stats: {
        totalIPs: poolSize,
  ...
  async manualSwitchMode(newMode, cooldownMinutes = 30) {
    ...
    const oldMode = await this.getConfiguredMode()
    await client.set(KEY.MODE, newMode)
    
    if (newMode === 'proxy' && cooldownMinutes > 0) {
      await client.set(KEY.MODE_COOLDOWN, Date.now() + cooldownMinutes * 60 * 1000)
    } else {
      await client.del(KEY.MODE_COOLDOWN)
    }
    ...
    return {
      success: true,
      message: `已切换到 ${newMode} 模式`,
      configuredMode: newMode,
      effectiveMode: await this.getCurrentMode()
    }
  }
}
