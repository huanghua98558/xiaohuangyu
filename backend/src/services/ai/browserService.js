        ...
        for (const c of result.comments) {
          ...
          if (textMatch && nameMatch) {
            ...
            if (taskAuthorName && pageAuthorName) {
              const authorCompare = matchAuthorWithTolerance(taskAuthorName, pageAuthorName, { allowMissingDetected: true })
              authorMatch = { matched: authorCompare.match, taskAuthorName, pageAuthorName, mode: authorCompare.mode }
              logger.info(`[抖音评论] 达人名匹配: 任务=${taskAuthorName}, 页面=${pageAuthorName}, 结果=${authorCompare.match}, 模式=${authorCompare.mode}`)
            }
