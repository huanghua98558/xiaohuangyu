...
export function verifyNickname(screenshotNickname, claimNickname, rules = DEFAULT_RULES.nickname) {
  ...
  // 预处理：去除 emoji/特殊符号，只保留中英文和数字，实现部分匹配
  const emojiRegex = /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE00}-\u{FE0F}\u{200D}\u{20E3}\u{E0020}-\u{E007F}]/gu;
  const normalize = (str) => {
    let result = str.trim();
    // 去除 emoji 和特殊表情符号
    result = result.replace(emojiRegex, '');
    // 去除 OCR 可能产生的乱码占位符（口、□、?、*等）
    result = result.replace(/[口□■◻◼◽◾▪▫?？*＊_＿]/g, '');
    result = result.trim();
    if (rules.ignoreCase !== false) {
      result = result.toLowerCase();
    }
    return result;
  };
  
  // 提取核心字符（仅中文+英文+数字）用于兜底匹配
  ...
  const core1 = extractCore(screenshotNickname);
  const core2 = extractCore(claimNickname);
  ...
  switch (rules.matchMode) {
    ...
    case 'contains':
      ...
      // 核心字符部分匹配（处理表情包/特殊字符导致 OCR 丢失的情况）
      if (core1 && core2 && (core1.includes(core2) || core2.includes(core1))) {
        return { passed: true, reason: '昵称核心字符匹配（忽略表情）', similarity: 0.75 };
      }
      // 降级到模糊匹配
      ...
      // 核心字符模糊匹配兜底
      if (core1 && core2) {
        const coreSim = calculateSimilarity(core1, core2);
        if (coreSim >= rules.minSimilarity) {
          return { passed: true, reason: '昵称核心模糊匹配（忽略表情）', similarity: coreSim };
        }
      }
