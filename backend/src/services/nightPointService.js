...
class NightPointService {
  normalizeCoefficientMapRows(rows = []) {
    const normalized = (rows || [])
      ...
      .filter(
        (item) =>
          Number.isFinite(item.online_users_max) &&
          Number.isFinite(item.coefficient) &&
          item.online_users_max >= 0
      )
      .sort((a, b) => {
        if (a.online_users_max !== b.online_users_max) {
          return a.online_users_max - b.online_users_max
        }
    ...
    return normalized
      .filter((item) => {
        if (seen.has(item.online_users_max)) {
          return false
        }
        seen.add(item.online_users_max)
        return true
      })
      .map((item, index) => ({
        ...item,
   ...
   */
  isNightTime() {
    const hour = getBusinessHour(new Date(), BUSINESS_TIME_ZONE)
  ...
  isNightTimeByReference(referenceTime, config = null) {
    const hour = getBusinessHour(referenceTime || new Date(), BUSINESS_TIME_ZONE)
    if (hour === null) {
      return false
    }
  ...
  async getConfig() {
    ...
    try {
      ...
      const config = await prisma.$queryRawUnsafe(`
        SELECT * FROM night_point_config ORDER BY id DESC LIMIT 1
      `)
  ...
  async getCoefficientMap() {
    ...
    try {
      ...
      const map = await prisma.$queryRawUnsafe(`
        SELECT * FROM online_user_coefficient_map ORDER BY online_users_max ASC, sort_order ASC, id ASC
      `)

      if (map && map.length > 0) {
        const result = this.normalizeCoefficientMapRows(map)
    ...
    // 返回默认映射
    const defaultMap = this.normalizeCoefficientMapRows([
      { id: 1, online_users_max: 5, coefficient: 1.8, description: '在线人数≤5', sort_order: 1 },
      { id: 2, online_users_max: 10, coefficient: 1.7, description: '在线人数≤10', sort_order: 2 },
      { id: 3, online_users_max: 20, coefficient: 1.6, description: '在线人数≤20', sort_order: 3 },
      { id: 4, online_users_max: 50, coefficient: 1.5, description: '在线人数≤50', sort_order: 4 },
      { id: 5, online_users_max: 999, coefficient: 1.4, description: '在线人数>50', sort_order: 5 }
    ])
  ...
  async calculateCoefficientByReference({
    ...
    if (!config?.is_active) {
      return { coefficient: 1.0, baseCoefficient: 1.0, bonus: 0, isNight: false, onlineUsers, config }
    }
    ...
    const normalizedOnlineUsers =
      onlineUsers === null || onlineUsers === undefined || onlineUsers === ''
        ? null
        : toSafeNumber(onlineUsers, null)

    if (normalizedOnlineUsers !== null) {
      for (const rule of coefficientMap) {
        if (normalizedOnlineUsers <= rule.online_users_max) {
    ...
    return {
      coefficient: roundPoints(coefficient),
      baseCoefficient: roundPoints(baseCoefficient),
      bonus: roundPoints(bonus),
      isNight: true,
      onlineUsers: normalizedOnlineUsers,
      config
    }
  }
  ...
  async getCurrentCoefficient(onlineUsers, basePoints = 100) {
    ...
    const breakdown = calculatePointBreakdown(basePoints, result.coefficient)
    ...
    return {
      ...
      displayPoints: breakdown.finalPoints,
  ...
  buildDescription(result) {
    if (result?.config?.is_active === false) {
      return '夜间积分未启用，按普通积分结算'
    }
    if (!result.isNight) {
      return '白天时段，正常积分'
    }
    const hour = getBusinessHour(new Date(), BUSINESS_TIME_ZONE) ?? 0
   ...
   * 更新系数映射配置
   */
  async updateCoefficientMap(id, payload) {
    const updateData =
      typeof payload === 'number'
        ? { coefficient: payload }
        : {
            ...(payload?.coefficient !== undefined ? { coefficient: Number(payload.coefficient) } : {}),
            ...(payload?.online_users_max !== undefined ? { online_users_max: Number(payload.online_users_max) } : {}),
  ...
  async updateConfig(configData) {
    const normalizedConfig = {
      ...(configData?.time_start !== undefined ? { time_start: Number(configData.time_start) } : {}),
      ...(configData?.time_end !== undefined ? { time_end: Number(configData.time_end) } : {}),
      ...(configData?.base_coefficient !== undefined ? { base_coefficient: Number(configData.base_coefficient) } : {}),
      ...(configData?.max_coefficient !== undefined ? { max_coefficient: Number(configData.max_coefficient) } : {}),
      ...(configData?.no_accept_bonus !== undefined ? { no_accept_bonus: Number(configData.no_accept_bonus) } : {}),
      ...(configData?.is_active !== undefined ? { is_active: Boolean(configData.is_active) } : {}),
    }
    ...
    if (queryError) {
      throw new Error('读取配置失败: ' + queryError.message)
    }
