...
async function loadRankings(startDate, endDate) {
  const result = await db.query(
    ...
    SELECT
      user_id,
      COALESCE(SUM(COALESCE(final_points, reward)), 0)::numeric AS total_points,
...
class LeaderboardSnapshotService {
  ...
  async generateSnapshot(type) {
    ...
    const allConfigs = await pointsRewardService.getConfigs()
    const rewardConfig = getRewardConfig(type, allConfigs)
    const rewardEnabled =
      type === 'monthly'
        ? allConfigs.rank_monthly_reward_enabled !== false
        : allConfigs.rank_weekly_reward_enabled !== false

    if (!rewardEnabled) {
      logger.info(
        `${type === 'monthly' ? '月榜' : '周榜'} ${meta.periodKey} 已关闭积分奖励，仅生成快照`
      )
    }

    const settledRewards = await db.transaction(async (client) => {
      const issued = []
      ...
      if (rewardEnabled) {
        ...
        for (let i = 0; i < Math.min(rankings.length, REWARD_LIMIT); i += 1) {
          ...
          issued.push({ userId, rank, points })
        }
      }
