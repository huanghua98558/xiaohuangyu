...
/** 成就展示与发放积分：≥10 的减半向下取整；1～9 不变；0 为 0 */
function scaleAchievementRewardPoints(raw) {
...
class AchievementService {
  ...
  async getAllAchievements() {
    ...
    return (achievements || []).map((a) => ({
      ...a,
      reward_points: scaleAchievementRewardPoints(readField(a, 'rewardPoints', 'reward_points')),
    }))
  }
  ...
  async getUserAchievements(userId) {
    ...
    return achievements.map((achievement) => ({
      ...achievement,
      reward_points: scaleAchievementRewardPoints(
        readField(achievement, 'rewardPoints', 'reward_points')
      ),
  ...
  async checkAndGrantAchievements(userId) {
    ...
    for (const achievement of achievements || []) {
      ...
      if (currentValue >= achievement.condition_value) {
        ...
        const rewardPts = scaleAchievementRewardPoints(
          readField(achievement, 'rewardPoints', 'reward_points')
        )
        ...
        if (rewardPts > 0) {
          ...
          const newPoints = oldPoints + rewardPts
          ...
          const { error: updateError } = await supabase
            ...
            .update({ 
              points: newPoints,
              total_points: Number(readField(user, 'totalPoints', 'total_points') || 0) + rewardPts
            })
            .eq('id', normalizedUserId)
        ...
        newlyGranted.push({ ...achievement, reward_points: rewardPts })
        logger.info(`用户 ${normalizedUserId} 获得成就: ${achievement.name}，奖励 ${rewardPts} 积分`)
  ...
  async getAchievementStats(userId) {
    ...
    if (achievedIds.length > 0) {
      ...
      totalRewardPoints =
        achievements?.reduce(
          (sum, a) =>
            sum + scaleAchievementRewardPoints(readField(a, 'rewardPoints', 'reward_points')),
          0
        ) || 0
    }
