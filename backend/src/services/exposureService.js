...
import { redisClient, REDIS_ENABLED, cache } from '../utils/redis.js'
...
import { getConfigValue } from './systemConfigService.js'
...
const DEFAULT_CONFIG = {
  ...
  priorityMode: {
    ...
    freshnessWeight: 1,      // 任务新鲜度权重
    remainWeight: 1,         // 剩余名额权重
    cityMatchWeight: 1       // 城市匹配权重
  },
  // V2.0 新增配置
  cityExposureLimit: 3,           // 城市曝光限制默认值
  provinceExposureLimit: 10,      // 省份曝光限制默认值
...
const PRIORITY_REGION_CACHE_TTL_SECONDS = 15
...
class ExposureService {
    ...
    try {
      ...
      return {
        ...
        priorityMode: {
          ...
          freshnessWeight: Number(data.freshness_weight) ?? 1,
          remainWeight: Number(data.remain_weight) ?? 1,
          cityMatchWeight: Number(data.city_match_weight) ?? 1
        },
        // V2.0 新增配置
        cityExposureLimit: Number(data.city_exposure_limit) ?? 3,
        provinceExposureLimit: Number(data.province_exposure_limit) ?? 10,
  ...
  async initTaskExposure(taskId, needCount) {
    try {
      ...
      const queuePosition = Number(maxPos?.queue_position ?? 0) + 1
  ...
  async recordView(taskId, userId, city, province, source = 'list') {
    try {
      const isNew = await db.transaction(async (client) => {
        const ins = await client.query(
          `INSERT INTO task_view_records (task_id, user_id, city, province, source)
      ...
      if (String(err.code) === '23505' || String(err.message || '').includes('23505')) {
        return { isNew: false }
      }
  ...
  async batchRecordViews(tasks, userId, city, province, source = 'list') {
    ...
    try {
      ...
      const viewedSet = new Set((viewedTasks || []).map((v) => String(v.task_id)))

      const newRows = taskIds
        .filter((taskId) => !viewedSet.has(String(taskId)))
        .map((taskId) => ({
        ...
        for (const row of newRows) {
          const ins = await client.query(
            `INSERT INTO task_view_records (task_id, user_id, city, province, source)
             VALUES ($1::BIGINT, $2::BIGINT, $3, $4, $5)
             ON CONFLICT (task_id, user_id) DO NOTHING`,
            [row.task_id, row.user_id, row.city, row.province, row.source]
          )
          if (ins.rowCount === 1) {
            await client.query(
              `UPDATE task_exposure
      ...
      if (inserted > 0) {
        await this.invalidatePriorityRegionStatsCache(taskIds, city, province)
      }
  ...
  async filterTasksByExposure(tasks, userId, city, province) {
    ...
    // 未登录：不做用户维度曝光（不写 task_view_records、不按个人浏览过滤）
  ...
  async assignQueuePosition(taskId) {
    try {
      ...
      const nextPosition = Number(maxPos?.queue_position ?? 0) + 1
  ...
  async filterTasksByExposureWithSequential(tasks, userId, city, province, config) {
    ...
    try {
      ...
      const exposedSet = new Set(exposedTaskIds.map((id) => String(id)))
      
      // 过滤任务
      const filteredTasks = tasks.filter((task) => exposedSet.has(String(task.id)))
      
      // 未登录：仅按顺序队列裁剪，不查个人浏览记录
      if (userId === null || userId === undefined || userId === '') {
  ...
  async updateConfig(config) {
    try {
      ...
      if (config.initialCoefficient !== undefined) {
        updateData.initial_coefficient = config.initialCoefficient
      }
      if (config.initialMinExtra !== undefined) {
        updateData.initial_min_extra = config.initialMinExtra
      }
      if (config.initialMaxExtra !== undefined) {
        updateData.initial_max_extra = config.initialMaxExtra
      }
      if (config.maxCoefficient !== undefined) {
        updateData.max_coefficient = config.maxCoefficient
      }
      if (config.checkIntervalMinutes !== undefined) {
        updateData.check_interval_minutes = config.checkIntervalMinutes
      }
      if (config.addRatioHigh !== undefined) {
        updateData.add_ratio_high = config.addRatioHigh
      }
      if (config.addRatioMid !== undefined) {
        updateData.add_ratio_mid = config.addRatioMid
      }
      if (config.addRatioLow !== undefined) {
        updateData.add_ratio_low = config.addRatioLow
      }
      if (config.rateThresholdHigh !== undefined) {
        updateData.rate_threshold_high = config.rateThresholdHigh
      }
      if (config.rateThresholdMid !== undefined) {
        updateData.rate_threshold_mid = config.rateThresholdMid
      }
      if (config.rateThresholdLow !== undefined) {
        updateData.rate_threshold_low = config.rateThresholdLow
      }
      ...
      if (config.provinceExposureLimit !== undefined) {
        updateData.province_exposure_limit = config.provinceExposureLimit
      }
      ...
      if (config.priorityMode !== undefined) {
        ...
        if (config.priorityMode.freshnessWeight !== undefined) {
          updateData.freshness_weight = config.priorityMode.freshnessWeight
        }
        if (config.priorityMode.remainWeight !== undefined) {
          updateData.remain_weight = config.priorityMode.remainWeight
        }
        if (config.priorityMode.cityMatchWeight !== undefined) {
          updateData.city_match_weight = config.priorityMode.cityMatchWeight
        }
      }
  ...
  getPriorityWeights(config = {}) {
    const activity = Math.max(0, Number(config.priorityMode?.activityWeight ?? DEFAULT_CONFIG.priorityMode.activityWeight))
    const speed = Math.max(0, Number(config.priorityMode?.speedWeight ?? DEFAULT_CONFIG.priorityMode.speedWeight))
    const completion = Math.max(0, Number(config.priorityMode?.completionWeight ?? DEFAULT_CONFIG.priorityMode.completionWeight))
    const total = activity + speed + completion

    if (total <= 0) {
      return {
        activity: DEFAULT_CONFIG.priorityMode.activityWeight,
        speed: DEFAULT_CONFIG.priorityMode.speedWeight,
        completion: DEFAULT_CONFIG.priorityMode.completionWeight
      }
    }
  ...
  getTaskPriorityWeights(config = {}) {
    return {
      freshness: Math.max(0, Number(config.priorityMode?.freshnessWeight ?? DEFAULT_CONFIG.priorityMode.freshnessWeight)),
      remain: Math.max(0, Number(config.priorityMode?.remainWeight ?? DEFAULT_CONFIG.priorityMode.remainWeight)),
      cityMatch: Math.max(0, Number(config.priorityMode?.cityMatchWeight ?? DEFAULT_CONFIG.priorityMode.cityMatchWeight))
    }
  }

  getPriorityRegionCacheKey(taskIds = [], city = null, province = null) {
    const normalizedTaskIds = [...new Set((taskIds || []).map((id) => String(id)).filter(Boolean))].sort()
    return `exposure:priority-region:${city || 'none'}:${province || 'none'}:${normalizedTaskIds.join(',')}`
  }

  async getPriorityRegionStats(taskIds = [], city = null, province = null) {
    const normalizedTaskIds = [...new Set((taskIds || []).map((id) => String(id)).filter(Boolean))]
    if (normalizedTaskIds.length === 0 || (!city && !province)) {
      return new Map()
    }

    const cacheKey = this.getPriorityRegionCacheKey(normalizedTaskIds, city, province)
    ...
    const rows = await db.queryMany(
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE $2::text IS NOT NULL AND city = $2) AS city_count,
        COUNT(*) FILTER (WHERE $3::text IS NOT NULL AND province = $3) AS province_count
      FROM task_view_records
      WHERE task_id = ANY($1::bigint[])
      GROUP BY task_id
      `,
      [normalizedTaskIds, city ?? null, province ?? null]
    )

    const statsEntries = (rows || []).map((row) => [
      String(row.task_id),
      {
    ...
    await cache.set(cacheKey, statsEntries, PRIORITY_REGION_CACHE_TTL_SECONDS)
    return new Map(statsEntries)
  }

  async invalidatePriorityRegionStatsCache(taskIds = [], city = null, province = null) {
    if (!taskIds || taskIds.length === 0) {
      return
    }
    const cacheKey = this.getPriorityRegionCacheKey(taskIds, city, province)
  ...
  async calculateTaskPriority(userId) {
    try {
      ...
      if (user.is_whitelist) {
        return Math.min(100, Math.max(0, 100 + Number(config.priorityMode?.whitelistBonus || 0)))
      }

      // 黑名单用户：最低优先级
      if (user.is_blacklist) {
        return Math.min(100, Math.max(0, 50 + Number(config.priorityMode?.blacklistPenalty || -50)))
      }
      ...
      const weights = this.getPriorityWeights(config)
      ...
      let speedScore = 0.5
      if (avgSubmitTime > 0) {
        if (avgSubmitTime <= 30) speedScore = 1
        else if (avgSubmitTime <= 60) speedScore = 0.8
        else if (avgSubmitTime <= 120) speedScore = 0.6
        else speedScore = 0.4
      }
      ...
      const lastActiveHours = user.last_task_date
        ...
        : 999
      let activityScore = 0
      if (lastActiveHours <= 1) activityScore = 1
      else if (lastActiveHours <= 6) activityScore = 0.8
      else if (lastActiveHours <= 24) activityScore = 0.5
      else if (lastActiveHours <= 72) activityScore = 0.2

      // 综合计算：等级基础分 + 可配置权重分
      const priority = Math.round(
        baseScore * 0.4 +
        completionRate * (weights.completion * 60) +
        speedScore * (weights.speed * 60) +
        activityScore * (weights.activity * 60)
      )
  ...
  async sortTasksByPriority(tasks, userId, city, province) {
    try {
      ...
      const config = await this.getConfig()
      const taskWeights = this.getTaskPriorityWeights(config)
      ...
      const tasksWithScore = await Promise.all(
        tasks.map(async (task) => {
          ...
          // 2. 地域匹配：本地任务加分
          if (task.city === city) {
            score += 15 * taskWeights.cityMatch
          } else if (task.province === province) {
            score += 8 * taskWeights.cityMatch
          }

          // 3. 剩余名额：名额多的任务更容易被领取，降权
          if (task.remain >= 50) {
            score -= 10 * taskWeights.remain
          } else if (task.remain >= 20) {
            score -= 5 * taskWeights.remain
          } else if (task.remain <= 5) {
            score += 10 * taskWeights.remain // 名额少，优先曝光
          }
          ...
          if (hours <= 1) {
            score += 10 * taskWeights.freshness // 新任务加分
          } else if (hours >= 24) {
            score -= 10 * taskWeights.freshness // 老任务降权
          }
  ...
  async getUserDynamicConcurrency(userId) {
    try {
      const { data: user, error } = await supabase
        .from('users')
        .select('level, exposure_level, is_whitelist')
        .eq('id', userId)
        .single()

      const globalDefault = Number(await getConfigValue('maxActiveTasksPerUser', 3))
      ...
      const { data: levelConfig } = await supabase
        .from('level_configs')
        .select('concurrent_tasks')
        .eq('level', userLevel)
        .maybeSingle()

      const levelConcurrency = Number(levelConfig?.concurrent_tasks || 0)
      const resolvedConcurrency = levelConcurrency > 0 ? levelConcurrency : globalDefault

      // 白名单只放大，不打破已有等级/全局规则
      if (user.is_whitelist) {
        return Math.max(resolvedConcurrency, globalDefault, 15)
      }

      return resolvedConcurrency
    } catch (err) {
      logger.error(`获取用户动态并发限制失败: ${err.message}`)
      return Number(await getConfigValue('maxActiveTasksPerUser', 3))
    }
  }
  ...
  async filterTasksByPriority(tasks, userId, city, province) {
    try {
      const config = await this.getConfig()
      const cityExposureLimit = Number(config.cityExposureLimit || DEFAULT_CONFIG.cityExposureLimit || 3)
      const provinceExposureLimit = Number(config.provinceExposureLimit || DEFAULT_CONFIG.provinceExposureLimit || 10)
      ...
      logger.debug(
        `[Exposure] priority sort user=${userId} activeClaims=${inProgressCount ?? 0} concurrencyLimit=${concurrencyLimit}`
      )

      if (Number(inProgressCount || 0) >= Number(concurrencyLimit || 0)) {
        logger.info(`[Exposure] user=${userId} 已达并发上限 ${inProgressCount}/${concurrencyLimit}，priority 模式暂停继续分发`)
        return []
      }
      ...
      const exposureQuery = supabase
        .from('task_exposure')
        .select('task_id, current_exposure, max_exposure, status')
        .in('task_id', taskIds)

      const viewedQuery = supabase
        .from('task_view_records')
        .select('task_id')
        .eq('user_id', userId)
        .in('task_id', taskIds)

      const regionStatsPromise = this.getPriorityRegionStats(taskIds, city, province)

      const [{ data: exposures }, { data: viewedTasks }, regionStats] = await Promise.all([
        exposureQuery,
        viewedQuery,
        regionStatsPromise
      ])

      const exposureMap = new Map((exposures || []).map(item => [item.task_id, item]))
      const viewedSet = new Set((viewedTasks || []).map(item => item.task_id))

      const filteredTasks = tasks.filter(task => {
        if (viewedSet.has(task.id)) {
          return true
        }
        ...
        if (Number(exposure.current_exposure || 0) >= Number(exposure.max_exposure || 0)) {
          return false
        }

        const regionStat = regionStats.get(String(task.id)) || { cityCount: 0, provinceCount: 0 }

        if (city && regionStat.cityCount >= cityExposureLimit) {
          return false
        }

        if (province && regionStat.provinceCount >= provinceExposureLimit) {
          return false
        }
  ...
  async calculateSelectionScore(userId) {
    try {
      ...
      const blacklistPenalty = Number(config.priorityMode?.blacklistPenalty || -50)
      const weights = this.getPriorityWeights(config)
      ...
      // 黑名单用户：最低优先级
      if (user.is_blacklist) {
        return Math.max(0, 50 + blacklistPenalty)
      }
      ...
      baseScore += completionRate * (weights.completion * 30)

      // 3. 提交速度贡献
      let speedScore = 0
      if (avgSubmitTime > 0) {
        if (avgSubmitTime <= 30) speedScore = weights.speed * 20
        else if (avgSubmitTime <= 60) speedScore = weights.speed * 15
        else if (avgSubmitTime <= 120) speedScore = weights.speed * 10
        else speedScore = weights.speed * 5
      }
      ...
      const lastActiveHours = user.last_task_date 
        ...
        : 999
      let activityScore = 0
      if (lastActiveHours <= 1) activityScore = weights.activity * 10
      else if (lastActiveHours <= 6) activityScore = weights.activity * 8
      else if (lastActiveHours <= 24) activityScore = weights.activity * 5
      else if (lastActiveHours <= 72) activityScore = weights.activity * 2
  ...
  async batchCalculateSelectionScores(userIds) {
    ...
    try {
      ...
      for (const user of users) {
        ...
        // 黑名单用户
        if (user.is_blacklist) {
          scoreMap.set(user.id, Math.max(0, 50 + blacklistPenalty))
          continue
        }
  ...
  async checkProvinceExposureLimit(taskId, province) {
    try {
      const config = await this.getConfig()
      const provinceLimit = Number(config.provinceExposureLimit || DEFAULT_CONFIG.provinceExposureLimit || 10)

      const { count: provinceExposure } = await supabase
        .from('task_view_records')
        .select('*', { count: 'exact', head: true })
        .eq('task_id', taskId)
        .eq('province', province)

      const remaining = Math.max(0, provinceLimit - (provinceExposure || 0))

      return {
        limit: provinceLimit,
        current: provinceExposure || 0,
        remaining,
        canExpose: remaining > 0
      }
    } catch (err) {
      logger.error(`检查省份曝光限制失败: ${err.message}`)
      return {
        limit: DEFAULT_CONFIG.provinceExposureLimit,
        current: 0,
        remaining: DEFAULT_CONFIG.provinceExposureLimit,
        canExpose: true
      }
    }
  }

  async checkTaskRegionExposureLimit(taskId, city, province) {
    try {
      const { data: task } = await supabase
        .from('tasks')
        .select('city_limit, province_limit')
        .eq('id', taskId)
        .single()

      const cityLimit = Number(task?.city_limit || 1)
      const provinceLimit = Number(task?.province_limit || 4)
      ...
      if (province) {
        const { count } = await supabase
          ...
          .eq('task_id', taskId)
          .eq('province', province)
        provinceCurrent = Number(count || 0)
      }
      ...
      return {
        city: {
          limit: cityLimit,
          current: cityCurrent,
          remaining: Math.max(0, cityLimit - cityCurrent),
          canExpose: city ? cityCurrent < cityLimit : true
        },
        province: {
          limit: provinceLimit,
          current: provinceCurrent,
          remaining: Math.max(0, provinceLimit - provinceCurrent),
          canExpose: province ? provinceCurrent < provinceLimit : true
        }
      }
    } catch (err) {
      logger.error(`检查任务地域曝光限制失败: ${err.message}`)
      return {
        city: { limit: 1, current: 0, remaining: 1, canExpose: true },
        province: { limit: 4, current: 0, remaining: 4, canExpose: true }
      }
    }
  }
  ...
  async shouldExposeTask(taskId, userId, city, province) {
    try {
      ...
      // 2. 检查任务城市曝光限制（曝光层使用后台全局配置，不直接吃任务领取名额）
      ...
      const provinceCheck = await this.checkProvinceExposureLimit(taskId, province)
      if (!provinceCheck.canExpose) {
        return {
          shouldExpose: false,
          reason: 'province_limit_reached',
          detail: `该省份曝光已达上限: ${provinceCheck.current}/${provinceCheck.limit}`
        }
      }
      ...
      return {
        ...
        detail: {
          ...
          provinceRemaining: provinceCheck.remaining,
          selectionScore,
