    ...
    try {
      ...
      const released = await db.transaction(async (client) => {
        const sel = await client.query(
          `
          SELECT id, task_id FROM claims
          WHERE status = $1 AND expires_at < $2::timestamptz
          ORDER BY id
          LIMIT 100
          FOR UPDATE
          `,
        ...
        await client.query(`UPDATE claims SET status = $1 WHERE id IN (${ph})`, [
          'expired',
          ...ids
        ])
        ...
        for (const r of rows) {
          ...
          taskRecovery.set(tid, (taskRecovery.get(tid) || 0) + 1)
        }
        ...
        for (const [taskId, count] of taskRecovery) {
          await client.query(
            `
            UPDATE tasks
            SET remain = LEAST(need_count, remain + $2::int), updated_at = NOW()
            WHERE id = $1::bigint
            `,
            [taskId, count]
          )
          await client.query(
            ...
            SET accepted_count = GREATEST(0, accepted_count - $2::int), updated_at = NOW()
        ...
        return { count: rows.length, taskCount: taskRecovery.size }
      })

      if (released.count > 0) {
        logger.debug(`[V3.0] 发现 ${released.count} 个过期任务需要释放`)
        logger.info(
          `[V3.0] 释放了 ${released.count} 个过期任务，涉及 ${released.taskCount} 个任务`
        )
      }
    } catch (err) {
