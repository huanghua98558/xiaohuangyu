...
class PrismaQueryBuilder {
  ...
  async _executeQuery() {
    ...
    // BigInt→Number 安全转换（防止 Prisma 类型拒绝）
    const sanitizeWhere = (obj) => {
    ...
    try {
      ...
      if (funcName === 'increment_task_remain') {
        ...
        await prisma.$executeRaw`
          UPDATE tasks
          SET remain = LEAST(need_count, remain + ${increment}), updated_at = NOW()
      ...
      if (funcName === 'decrement_task_exposure') {
        const decrement = Number(params.decrement_by || params.decrement || 1)
        const ids = params.task_ids ?? params.taskIds
        if (Array.isArray(ids) && ids.length > 0) {
          for (const tid of ids) {
            await prisma.$executeRaw`
              ...
              SET current_exposure = GREATEST(current_exposure - ${decrement}, 0),
                  updated_at = NOW()
              WHERE task_id = ${BigInt(String(tid))}
            `
          }
        } else {
          const taskId = params.task_id || params.p_task_id
          await prisma.$executeRaw`
            ...
            SET current_exposure = GREATEST(current_exposure - ${decrement}, 0),
                updated_at = NOW()
            WHERE task_id = ${BigInt(String(taskId))}
          `
        }
        return { data: { success: true }, error: null }
      }
      ...
      if (funcName === 'increment_exposure') {
        const taskId = params.task_id || params.p_task_id
        const increment = Number(params.increment || 1)
        await prisma.$executeRaw`
          UPDATE task_exposure
