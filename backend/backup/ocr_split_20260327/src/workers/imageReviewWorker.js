...
function mergeOcrEvidence(combinedEvidence, ocrResult, screenshotIndex) {
  ...
  // 判断是否是达人主页（有 @开头的达人名字）
  const hasAuthorTag = ocrResult.ocr_text && /@[^\s]+/.test(ocrResult.ocr_text);
  
  // 判断是否是用户自己的评论截图：
  // 1. ocrResult.author 非空 → 说明昵称行有我字（OCR服务已处理），是用户自己的评论
  // 2. 或者 OCR 文本中有条评论三个字 → 评论列表页面
  const hasNicknameWithMe = Boolean(detectedName && ocrResult.has_comment_keyword);
  const hasCommentCount = ocrResult.ocr_text && ocrResult.ocr_text.includes('条评论');
  
  // 如果是达人主页（有@标签），则不是评论截图，需要调用 YOLO
  let isCommentScreenshot = Boolean(hasNicknameWithMe || hasCommentCount);
  if (hasAuthorTag) {
    console.log('[Worker] 检测到 @ 标签，识别为达人主页，需要调用 YOLO');
    isCommentScreenshot = false;
  }
  
  // 如果有评论字样但既没有昵称我也没有条评论 → 不是用户自己的评论，直接拒绝
  const shouldReject = ocrResult.has_comment_keyword && !hasNicknameWithMe && !hasCommentCount && !hasAuthorTag;
  ...
  // 标记是否是用户自己的评论
  combinedEvidence.isUserComment = hasNicknameWithMe;
  // 标记是否应该直接拒绝（不是用户自己的评论）
  combinedEvidence.shouldReject = combinedEvidence.shouldReject || shouldReject;
...
class ImageReviewWorker {
  ...
  evaluateResult(ocrResult, yoloResult, item) {
    ...
    // 如果应该直接拒绝（不是用户自己的评论），立即返回拒绝结果
    if (ocrResult.shouldReject) {
      console.log('[Worker] 检测到非用户自己的评论，直接拒绝');
      return {
        passed: false,
        reasons: ['请上传自己评论的截图'],
        type: ocrResult.type,
