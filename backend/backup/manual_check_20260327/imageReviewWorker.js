...
function mergeOcrEvidence(combinedEvidence, ocrResult, screenshotIndex, expectedRole = 'homepage') {
  ...
  combinedEvidence.screenshots.push({
    ...
    commenterNickname: isCommentScreenshot ? (commenterNickname || '没有') : '没有',
...
async function getNextOcrService(role = 'homepage') {
  try {
    ...
    const healthyServices = OCR_SERVICES_CONFIG.filter(
      (service) => servicePool.includes(service.url) && service.healthy
    );
    if (healthyServices.length === 0) {
      return servicePool[serviceIndex];
    }
...
async function callOCR(imagePath, screenshotRole = 'homepage') {
  ...
  const healthyServices = poolServices.filter((service) => service.healthy);
  const fallbackServices = OCR_SERVICES_CONFIG.filter(
    (service) => !poolUrls.includes(service.url) && service.healthy
  );
  ...
  OCR_SERVICES_CONFIG.forEach(pushService);
  ...
  async processItem(item) {
    ...
    try {
      ...
      for (let i = 0; i < screenshots.length; i++) {
        ...
        // 调用 OCR
        const ocrResult = await callOCR(imagePath, expectedRole);
      ...
      if (!result.passed) {
        aiReReview = await this.runAiReReview(screenshotPaths, item, result);

        if (aiReReview?.decision === 'approved' && aiReReview.result) {
          result = {
            ...aiReReview.result,
            source: aiReReview.result.source || 'ai_review',
            recheck: aiReReview.result.recheck || {
              previousReasons: result.reasons || [],
              screenshotCount: screenshotPaths.length
            }
          };
        } else if (aiReReview?.decision === 'rejected' && aiReReview.result) {
          result = {
            ...aiReReview.result,
            passed: false,
            source: aiReReview.result.source || 'ai_review',
        ...
        } else if (aiReReview?.decision === 'manual') {
          result = {
            ...result,
            passed: false,
            source: 'ai_review_failed',
            reasons: [...new Set([...(result.reasons || []), aiReReview.reason || 'AI复审未产出有效识别结果'])],
            recheck: {
              previousReasons: result.reasons || [],
              screenshotCount: screenshotPaths.length,
              decision: 'manual'
      ...
      if (!result.passed) {
        const manualReason =
          aiReReview?.decision === 'manual'
            ? 'AI复审未产出有效结果，已加入人工检查列表'
            : 'AI复审未通过，已加入人工检查列表';
        await this.rejectTask(item, result, {
