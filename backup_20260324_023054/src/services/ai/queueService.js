...
export async function enqueueReview(claimId, options = {}) {
  try {
    ...
    if (claimError || !claim) {
      logger.error('获取Claim失败:', claimError)
    ...
    logger.info(`审核任务入队: claimId=${claimId}, queueId=${queueItem.id}`)
...
export async function processQueueItem(item, config = {}) {
  ...
  try {
    ...
    // 决定处理方式（新逻辑：根据auto_reject_enabled决定）
    let decision = 'manual'
    let reason = analysisResult.details || ''
    
    const approveThreshold = config.approveThreshold || reviewerConfig.approveThreshold || 0.85
    const rejectThreshold = config.rejectThreshold || reviewerConfig.rejectThreshold || 0.50
    
    // 如果需要随机抽查，强制进入人工
    if (shouldRandomCheck) {
      decision = 'manual'
      reason = `随机抽查: ${analysisResult.details}`
    } else if (analysisResult.passed && confidence >= approveThreshold) {
      // 通过条件：截图审核通过 + 置信度达到阈值
      decision = 'approved'
      ...
      if (autoRejectEnabled) {
        ...
        // 人工复核模式：转人工审核
        decision = 'manual'
        reason = `需人工复核: ${analysisResult.rejectionReason || analysisResult.details}（置信度：${(confidence * 100).toFixed(1)}%）`
      }
    } else {
      // passed=true 但置信度不够（边界情况）
      if (autoRejectEnabled) {
        decision = 'approved'
        reason = `AI自动通过（置信度边界）: ${analysisResult.details}`
      } else {
        decision = 'manual'
        reason = `需人工复核（置信度不足）: ${analysisResult.details}（置信度：${(confidence * 100).toFixed(1)}%）`
      }
    }
    ...
    if (decision !== 'manual') {
      ...
      await supabase
        .from('claims')
        .update({
          status: decision === 'approved' ? 'done' : 'doing',  // 拒绝后回到doing状态，允许重新提交
          ...
          proof_images: decision === 'approved' ? undefined : null  // 拒绝时清空截图
        })
        .eq('id', claim_id)
      ...
      if (decision === "approved") {
        ...
        if (videoUrl && commentContent) {
          ...
          // 加入链接审核队列
          try {
            await enqueueLinkVerification({
              claimId: claim_id,
              userId: user_id,
              taskId: item.task_id,
              videoUrl: videoUrl,
              comment: commentContent,
              userName: claim.platform_nickname || "",
              platform: platform,
              taskAuthorName: taskAuthorName
            })
            logger.info(`[审核] Claim ${claim_id} 已加入链接审核队列`)
          } catch (linkError) {
...
async function approveClaimWithReward(claimId) {
  ...
  const reward = claim.reward || 0
  
  if (reward <= 0) {
    logger.info('[积分发放] claimId=' + claimId + ' 无需发放积分')
    return
  }
  
  // 🔧 修复：使用 PostgreSQL 原子操作更新积分
  // 使用 supabase.rpc 调用自定义函数或直接使用 SQL
  const { error: updateError } = await supabase.rpc('increment_user_points', {
    user_id_param: claim.user_id,
    reward_amount: reward
  })
  
  if (updateError) {
    // 如果 RPC 不存在，回退到直接更新（带版本检查）
    logger.warn('[积分发放] RPC 不可用，使用直接更新:', updateError.message)
    ...
    if (user) {
      ...
      const newTotalPoints = (user.total_points || 0) + reward
      ...
      await supabase
        ...
        .insert({
          user_id: claim.user_id,
          old_points: user.points || 0,
          new_points: newPoints,
          change: reward,
          balance: newPoints,
          type: 'task',
          description: '完成任务: ' + (claim.tasks?.title || '未知任务'),
          related_id: claimId
        })
    }
  } else {
    ...
    logger.info('[积分发放] claimId=' + claimId + ' 已发放积分 ' + reward)
  }
}
