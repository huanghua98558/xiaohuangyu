...
const centerLink = computed(() => {
  // 体验官和管理员显示"我的任务"，发布者和审核员显示"任务管理"
  if (user.value?.role === 'part_timer' || user.value?.role === 'admin') {
    return '/my/tasks'
  }
...
.tab-item.active .tab-icon {
  transform: scale(1.1);
  filter: drop-shadow(0 2px 8px rgba(83, 109, 254, 0.4));
}
