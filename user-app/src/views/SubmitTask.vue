  ...
  <div class="yx-page no-tabbar submit-page">
    <header class="yx-header center">
      ...
      <div class="yx-header-main">
        ...
        <p class="yx-subtitle">请按顺序上传 2 张截图。</p>
      </div>
      <div class="yx-icon-btn">📤</div>
    </header>

    <div class="yx-empty" v-if="errorMessage && !claim">
      <strong>暂时无法重新提交</strong>
      <span>{{ errorMessage }}</span>
      <button class="yx-btn" type="button" @click="router.push({ path: '/my/tasks', query: { tab: 'doing' } })">返回我的任务</button>
    </div>

    <div v-else-if="claim">
      <section class="yx-hero-card">
        <h2 class="submit-title">{{ claim.title }}</h2>
        <p class="submit-sub">上传 2 张截图后提交审核。</p>
      ...
      <section class="yx-card">
        <div class="yx-card-head">
          <div>
            <h3>上传顺序</h3>
            <div class="yx-card-note">共 2 张</div>
          </div>
        </div>
        ...
        <p class="yx-note" style="margin-top:8px;">传反、漏传或截图不完整会影响审核结果。</p>
      </section>
        ...
        <article v-for="slotIndex in 2" :key="slotIndex - 1" class="yx-card">
          ...
          <div class="compare-grid">
            <div class="compare-col">
              <span class="compare-label">示例图</span>
              <button
            ...
            <div class="compare-col">
              <span class="compare-label">上传截图</span>
...
onMounted(async () => {
  ...
  if (!found) {
    errorMessage.value = '任务不存在或已失效，请返回我的任务查看最新状态。'
    return
  }
  if (found.status !== 'doing' && !found.canResubmit) {
    errorMessage.value = found.status === 'expired'
      ? '该任务已过期，不能重新提交，请返回我的任务查看最新状态。'
      : '该任务当前不能重新提交，请返回我的任务查看最新状态。'
    return
  }
  claim.value = found
...
function removeImg(i) {
  ...
  // 用新数组引用替换，确保 Vue 响应式更新不会出现中间态导致白屏
  const newPreviews = [...previewImages.value]
  newPreviews[i] = null
  previewImages.value = newPreviews
  const newScreenshots = [...screenshots.value]
  newScreenshots[i] = null
  screenshots.value = newScreenshots
  // 等 DOM 更新后直接打开文件选择器，避免中间态闪烁
  nextTick(() => triggerFile(i))
}
...
.rule-item {
  padding: 9px 11px;
  border-radius: 12px;
  background: rgba(33,48,75,0.035);
  border: 1px solid rgba(33,48,75,0.06);
  font-size: 13px;
  font-weight: 700;
}
