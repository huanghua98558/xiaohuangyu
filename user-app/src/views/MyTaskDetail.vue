<template>
  <div class="yx-page no-tabbar my-task-detail-page">
    ...
    <div class="content" v-else-if="task && claim">
      ...
      <div class="reject-reason yx-card" v-if="claim.reviewNote && (isRejected || isManual || isReleased)">
        <div class="yx-card-head-bar">
          <h4>{{ isManual ? '人工复审说明' : '处理说明' }}<span class="yx-card-note">查看本次处理原因</span></h4>
        </div>
      ...
      <div class="blocked-record yx-card" v-if="isBlockedConfirmed">
        <div class="yx-card-head-bar">
          <h4>评论封控记录<span class="yx-card-note">本次账号状态说明</span></h4>
        </div>
      ...
      <div class="yx-card tutorial-card" v-if="qualificationStandards.length">
        <div class="yx-card-head-bar">
          <h3>任务合格标准说明<span class="yx-card-note">提交前逐项自查</span></h3>
        </div>
        <div class="tutorial-tip-box">
          <strong>重点提醒：</strong>
          <span>视频截图、评论截图、点赞收藏、本人评论、评论字数和内容导向要同时满足。</span>
        </div>
      ...
      <div class="yx-card tutorial-card" v-if="positiveCommentExamples.length">
        <div class="yx-card-head-bar">
      ...
      <div class="yx-card tutorial-card" v-if="commentLibrary.length">
        <div class="yx-card-head-bar">
        ...
        <div class="comment-library-entry">
          ...
          <button class="yx-btn" @click="openCommentLibrary">打开参考评论库</button>
        </div>
      </div>
      ...
      <div class="submitted-info yx-card" v-if="showSubmittedInfo">
        <div class="yx-card-head-bar">
          <h3>提交信息<span class="yx-card-note">查看你已提交的内容</span></h3>
        </div>
        <div class="info-item">
          <label>平台昵称</label>
      ...
      <div class="submitted-info yx-card" v-if="simpleReviewHistory.length">
        <div class="yx-card-head-bar">
          <h3>处理记录<span class="yx-card-note">查看审核过程</span></h3>
        </div>
    ...
    <div class="image-preview" v-if="showCommentLibrary" @click="closeCommentLibrary">
      <button type="button" class="close" @click.stop="closeCommentLibrary">关闭</button>
      <div class="image-preview-stage comment-library-stage" @click.stop>
...
function openCommentLibrary() {
  showCommentLibrary.value = true
}

function closeCommentLibrary() {
...
async function copyComment(content) {
  try {
...
.title { font-size: 22px; margin: 0 0 10px; color: var(--yx-deep); }
...
.reward { color: #e55436; font-weight: 700; font-size: 20px; }
