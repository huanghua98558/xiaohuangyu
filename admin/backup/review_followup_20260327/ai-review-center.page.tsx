...
function ReviewHistoryPanel({ history, rejectCount }: { history?: Array<{ stage?: string; action: string; reason?: string; details?: any; data?: any; timestamp: string }>; rejectCount?: number }) {
  ...
  const getActionLabel = (action: string, stage?: string) => {
    // 根据 stage 和 action 组合翻译
    ...
    const actionLabels: Record<string, string> = {
      ...
      'approved': '审核通过',
      'rejected': '审核拒绝',
      'rejected_to_user': '退回用户修改',
      'manual': '转人工复审',
      'returned': '退回用户',
      'released': '任务释放',
      'completed': '任务完成',
      'image_approved': '图片审核通过',
      'image_rejected': '图片审核未通过',
      'link_approved': '链接验证通过',
      'link_rejected': '链接验证未通过'
    };
    
    // 优先使用完整翻译
    const fullKey = `${stage}_${action}`;
    if (actionLabels[fullKey]) return actionLabels[fullKey];
    
    // 组合翻译
    const stageLabel = stage ? stageLabels[stage] || stage : '';
    const actionLabel = actionLabels[action] || action;
    
    return stageLabel ? `${stageLabel} · ${actionLabel}` : actionLabel;
  };
              ...
              {(selectedItem.ai_confidence || selectedItem.ai_reason || selectedItem.image_review_reason || selectedItem.link_review_reason) && (
                <div className="p-3 rounded bg-muted/50 text-sm space-y-2">
                  ...
                  {selectedItem.ai_reason && (() => {
                    try {
                      const reason = typeof selectedItem.ai_reason === 'string' 
                        ? JSON.parse(selectedItem.ai_reason) 
                        : selectedItem.ai_reason;
                      
                      return (
                        <div className="p-2 rounded bg-blue-50 text-xs space-y-1">
                          <div className="font-medium flex items-center gap-1 text-blue-700">
                            <Bot className="w-3 h-3" /> AI审核结果
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={cn(
                              "px-1.5 py-0.5 rounded font-medium",
                              reason.passed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                            )}>
                          ...
                          {reason.reasons && reason.reasons.length > 0 && (
                            <div className="text-red-600">
                              原因: {reason.reasons.join(", ")}
                            </div>
                          )}
                          {reason.hasComment !== undefined && (
                            <div className="flex gap-3 text-gray-600">
                              <span>评论检测: {reason.hasComment ? "✅" : "❌"}</span>
                              {reason.author && <span>达人: {reason.author}</span>}
                            </div>
                          )}
                          {reason.interaction && (
                            <div className="flex gap-3 text-gray-600">
                              <span>点赞: {reason.interaction.hasLike ? "✅" : "❌"}</span>
                              <span>收藏: {reason.interaction.hasFavorite ? "✅" : "❌"}</span>
                              <span>关注: {reason.interaction.hasFollow ? "✅" : "❌"}</span>
                            </div>
                          )}
                        </div>
                      );
                ...
                <div className="p-3 rounded bg-muted/30 space-y-3">
                  ...
                  {selectedItem.review_history && selectedItem.review_history.length > 0 && (
                    <div className="space-y-2 mt-3">
                      <div className="text-sm font-medium text-muted-foreground">操作历史</div>
                      <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                        {selectedItem.review_history.map((h: any, i: number) => {
                          // 获取原因
                          const reason = h.reason || h.details?.reason || h.data?.reason || '';
                          // 获取详情
                          const detailText = h.details?.source || h.details?.rejectCount ? 
                            `[${h.details.source || ''}${h.details.rejectCount ? ` 拒绝${h.details.rejectCount}次` : ''}]` : '';
                          
                          return (
                            <div key={i} className="flex items-start gap-3 p-2.5 bg-muted/30 rounded-lg text-xs border">
                              <div className="flex-1 min-w-0">
                                <div className={cn(
                                  "font-medium flex items-center gap-1",
                                  h.action?.includes('rejected') ? "text-red-600" :
                                  h.action?.includes('approved') || h.action?.includes('completed') ? "text-green-600" : 
                                  h.action?.includes('returned') ? "text-orange-600" : "text-gray-700"
                                )}>
                                  {h.action === 'approved' && h.stage === 'image_review' ? '✅ 图片审核通过' :
                                   h.action === 'rejected' && h.stage === 'image_review' ? '❌ 图片审核未通过' :
                                   h.action === 'approved' && h.stage === 'link_review' ? '✅ 链接验证通过' :
                                   h.action === 'rejected' && h.stage === 'link_review' ? '❌ 链接验证未通过' :
                                   h.action === 'returned' ? '🔄 退回用户修改' :
                                   h.action === 'released' ? '⚠️ 任务已释放' :
                                   h.action === 'queued' ? '📥 进入审核队列' :
                                   h.action === 'completed' ? '🎉 任务完成' :
                                   h.action === 'manual' ? '👤 转人工审核' :
                                   h.action}
                                </div>
                                {reason && (
                                  <div className="text-muted-foreground mt-1 break-words">{reason}</div>
                                )}
                                {h.details?.interaction && (
                                  <div className="text-muted-foreground mt-1 flex gap-3 flex-wrap">
                                    <span>点赞: {h.details.interaction.hasLike ? '✅' : '❌'}</span>
                                    <span>收藏: {h.details.interaction.hasFavorite ? '✅' : '❌'}</span>
                                    <span>关注: {h.details.interaction.hasFollow ? '✅' : '❌'}</span>
                                  </div>
                                )}
                                {h.details?.hasComment !== undefined && (
                                  <div className="text-muted-foreground mt-1">
                                    评论检测: {h.details.hasComment ? '✅ 已检测到' : '❌ 未检测到'}
                                    {h.details.author && ` (达人: ${h.details.author})`}
                                  </div>
                                )}
                                {h.details?.type && (
                                  <div className="text-muted-foreground mt-1">截图类型: {h.details.type === 'A' ? '评论截图' : '达人主页'}</div>
                                )}
                              </div>
                              <div className="text-muted-foreground shrink-0 text-[10px] whitespace-nowrap">
