            ...
            <div className="space-y-3 text-xs overflow-y-auto flex-1">
              ...
              <div className="bg-gray-50 p-2 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                  ...
                  <div className="flex items-center gap-1"><Coins className="h-3 w-3 text-gray-400" /><span className="text-gray-400">奖励：</span><span className="text-primary font-bold">{formatPoints(claimDetail.settlement?.finalPoints ?? claimDetail.reward)}积分</span></div>
                  {claimDetail.settlement && (
                    <div className="col-span-2 text-xs text-gray-500">
                      积分明细：基础 {formatPoints(claimDetail.settlement.basePoints)}
                      {Number(claimDetail.settlement.coefficient || 1) > 1 ? (
                        <> + 加成 {formatPoints(claimDetail.settlement.bonusPoints)} (x{formatPoints(claimDetail.settlement.coefficient)})</>
                      ) : null}
                      {' '}= 实发 {formatPoints(claimDetail.settlement.finalPoints)}
                    </div>
                  )}
                </div>
              </div>
