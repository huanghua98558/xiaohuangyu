...
export default function NightPointsPage() {
  ...
  const [editedCoefficients, setEditedCoefficients] = useState<Record<number, number>>({})
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      setConfig({ ...config!, ...editedConfig } as NightPointConfig)
      setEditedConfig({})
    } catch (err) {
  ...
  const handleSaveCoefficient = async (id: number) => {
    const coefficient = editedCoefficients[id]
    ...
    try {
      ...
      setCoefficientMap(coefficientMap.map(m => 
        m.id === id ? { ...m, coefficient } : m
      ))
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        max="2"
                        className="w-24"
                        value={editedCoefficients[item.id] ?? item.coefficient}
                      ...
                      <Badge variant="outline">
                        x{editedCoefficients[item.id] ?? item.coefficient}
                      </Badge>
                    </div>
                  </TableCell>
