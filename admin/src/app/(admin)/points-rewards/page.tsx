...
interface Configs {
  ...
  // 周/月榜是否发放积分（关则仍生成快照）
  rank_weekly_reward_enabled: boolean
  rank_monthly_reward_enabled: boolean
  // 兑换限制开关
...
export default function PointsRewardsPage() {
  ...
  const fetchRewards = async (page = 1) => {
    try {
      ...
      if (data.code === 0) {
        setRewards((prev) => {
          if (page <= 1) return data.data.list
          const merged = [...prev, ...data.data.list]
          const deduped = new Map(merged.map((item: RewardItem) => [item.id, item]))
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
              ...
              {configs && (
                <div className="grid grid-cols-2 gap-4 py-4 overflow-y-auto flex-1 min-h-0">
                  ...
                  <div className="space-y-2 flex items-center justify-between col-span-2">
                    <div>
                      <Label>周榜发放积分奖励</Label>
                      <p className="text-xs text-muted-foreground mt-1">
                        关闭后仍会生成周榜快照，不向用户发放积分、不发送奖励通知
                      </p>
                    </div>
                    <Switch
                      checked={configs.rank_weekly_reward_enabled !== false}
                      onCheckedChange={(checked) =>
                        setConfigs({ ...configs, rank_weekly_reward_enabled: checked })
                      }
                    />
                  </div>
                  {/* 月榜奖励配置 */}
                  ...
                  <div className="space-y-2 flex items-center justify-between col-span-2">
                    <div>
                      <Label>月榜发放积分奖励</Label>
                      <p className="text-xs text-muted-foreground mt-1">
                        关闭后仍会生成月榜快照，不向用户发放积分、不发送奖励通知
                      </p>
                    </div>
                    <Switch
                      checked={configs.rank_monthly_reward_enabled !== false}
                      onCheckedChange={(checked) =>
                        setConfigs({ ...configs, rank_monthly_reward_enabled: checked })
                      }
                    />
                  </div>
                </div>
              ...
              {rewards.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="max-w-[220px]">
                    ...
                    {r.rewardDetail?.claimId && (
                      <div className="text-xs text-muted-foreground mt-1">
                        claim #{r.rewardDetail.claimId}
                      </div>
                    )}
                  </TableCell>
                  <TableCell className="max-w-[260px] text-xs text-muted-foreground">
                    {r.rewardDetail?.settlement ? (
                      <div className="space-y-1">
                        <div>
                          基础 {formatPoints(r.rewardDetail.settlement.basePoints)}
                          {r.rewardDetail.settlement.coefficient > 1 ? (
                            <> + 加成 {formatPoints(r.rewardDetail.settlement.bonusPoints)} (x{formatPoints(r.rewardDetail.settlement.coefficient)})</>
                          ) : null}
                        </div>
                        <div>
                          实发 {formatPoints(r.rewardDetail.settlement.finalPoints)}
                          {r.rewardDetail.recordedPoints !== r.rewardDetail.displayPoints && (
                            <>，原流水 {r.rewardDetail.recordedPoints > 0 ? '+' : ''}{formatPoints(r.rewardDetail.recordedPoints)}</>
                          )}
                        </div>
                        {r.type === 'task_adjustment' && (
                          <div>
                            修正 {r.rewardDetail.adjustmentPoints && r.rewardDetail.adjustmentPoints > 0 ? '+' : ''}
                            {formatPoints(r.rewardDetail.adjustmentPoints)}
                            {r.rewardDetail.previousFinalPoints ? (
                              <>，从 {formatPoints(r.rewardDetail.previousFinalPoints)} 调整到 {formatPoints(r.rewardDetail.expectedFinalPoints)}</>
                            ) : null}
                          </div>
                        )}
                      </div>
                  ...
                  <TableCell className={`text-right font-medium ${r.points > 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {Number(r.rewardDetail?.displayPoints ?? r.points) > 0 ? '+' : ''}
                    {formatPoints(r.rewardDetail?.displayPoints ?? r.points)}
                    {r.rewardDetail && r.rewardDetail.recordedPoints !== r.rewardDetail.displayPoints && (
