...
class PrismaQueryBuilder {
  ...
  async _executeQuery() {
    ...
    if (!this.updateData && !this.insertData && !this.rpcFunction && !this.deleteMode) {
          ...
          if (f.includes('!')) {
            ...
            args.select[rel] = {
              select: cleanCols.split(',').reduce((acc, col) => {
                // 下划线转驼峰
                acc[snakeToCamel(col.trim())] = true
                return acc
              }, {})
            }
          } else {
            // 下划线转驼峰 (task_id -> taskId)
            const camelField = snakeToCamel(f)
            args.select[camelField] = true
          }
        })
      }
      ...
      if (this.orderByField) {
        // 处理关联排序 (tasks.created_at)
        if (this.orderByField.includes('.')) {
          const [rel, field] = this.orderByField.split('.')
          args.orderBy = {
            [rel]: {
              [field]: this.orderByAsc ? 'asc' : 'desc'
            }
          }
        } else {
          args.orderBy = {
            [this.orderByField]: this.orderByAsc ? 'asc' : 'desc'
          }
        }
      }
    ...
    if (this.updateData) {
      // 将下划线字段名转换为驼峰命名
      const camelData = snakeKeysToCamel(this.updateData)
    ...
    if (this.insertData) {
      // 将下划线字段名转换为驼峰命名
      let camelData = this.insertData
      if (Array.isArray(this.insertData)) {
        camelData = this.insertData.map(snakeKeysToCamel)
      } else {
        camelData = snakeKeysToCamel(this.insertData)
      }
  ...
  async _executeRawSQL() {
    ...
    if (!this.updateData && !this.insertData && !this.deleteMode) {
      ...
      const orderBySQL = this.orderByField ? `ORDER BY ${this.orderByField} ${this.orderByAsc ? 'ASC' : 'DESC'}` : ''
    ...
    if (this.insertData) {
      if (Array.isArray(this.insertData)) {
        ...
        const mutation = `INSERT INTO ${tableName} (${keys}) VALUES ${values.join(', ')}`
        ...
        const mutation = `INSERT INTO ${tableName} (${keys.join(', ')}) VALUES (${values.join(', ')})`
        await this._executeRawMutation(mutation, Object.values(snakeData))
        return snakeData
      }
    }
    ...
    for (const cond of this.whereConditions) {
      ...
      switch (cond.type) {
        case 'eq':
          ...
          break
        case 'in':
          where[field] = { in: cond.values }
          break
        case 'gte':
          where[field] = { gte: cond.value }
          break
        case 'gt':
          where[field] = { gt: cond.value }
          break
        case 'lte':
          where[field] = { lte: cond.value }
          break
        case 'lt':
