...
export interface AdminLoginCaptcha {
  captchaId: string
  svg: string
}

/** 获取管理端登录图形验证码（4 位数字，SVG） */
export async function fetchAdminLoginCaptcha(): Promise<AdminLoginCaptcha> {
  const res = await fetch(`${API_BASE}/auth/captcha`, { method: 'GET' })
  const json = (await res.json()) as ApiResponse<AdminLoginCaptcha>
  if (json.code !== 0) {
