...
const SYSTEM_CONFIG_TTL = 60 * 1000
...
async function getSystemConfigValues(keys = []) {
  const now = Date.now()
  const result = {}
  const missingKeys = []
  ...
  for (const key of keys) {
    ...
    if (cached && now - cached.updatedAt < SYSTEM_CONFIG_TTL) {
...
async function shouldBroadcastAdminNotification() {
  const configs = await getSystemConfigValues([
...
export async function sendUserNotification(params) {
  ...
  const notification = await insertNotification('user_notifications', {
    user_id: userId,
    type,
    title,
    content: content || message || '',
...
export async function getUserNotifications(params = {}) {
  ...
  const result = await getNotificationList('user_notifications', {
    whereClauses,
    values,
    unreadOnly,
    page,
...
export async function markUserNotificationRead(notificationId, userId) {
  const columns = await getTableColumnSet('user_notifications')
  const sets = ['is_read = true']
  if (columns.has('read_at')) {
    sets.push('read_at = NOW()')
  }
